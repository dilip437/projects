...
//    alert("connect_and_pay =" + $(".amount-class").val());
    ...
    var pin = $(".pin-class").val();
    var card_number = $(".card-number").val();
    
    //alert("connect_and_pay =" + amount + " " + card_number + " " + pin);
    
//  http://localhost:8580/pos/pay?cardNumber=4532299071753039&pin=4567&amount=1400.00
    
	$.ajax({
	    type: "GET",
	    url: "http://localhost:8580/pos/pay" + "?cardNumber=" + card_number + "&pin=" + pin + "&amount=" + amount,
	    success: function (data) {
    		if(data) pay_success(data);
	    },
	    error: function (e) {
		//alert("Error Find User :: error =" + e.responseText);
		$("#screen-table-wait").css({"display": "block"});
